...
class OpenGraph implements Iterator
{
   ...
   */
	static private function _parse($HTML) {
		...
		foreach ($tags AS $tag) {
			if ($tag->hasAttribute('property') && mb_strpos($tag->getAttribute('property'), 'og:') === 0) {
				$key = mb_strstr(mb_substr($tag->getAttribute('property'), 3), '-', '_');
				$page->_values[$key] = $tag->getAttribute('content');
			}
			
			//Added this if loop to retrieve description values from sites like the New York Times who have malformed it. 
			if ($tag ->hasAttribute('value') && $tag->hasAttribute('property') && mb_strpos($tag->getAttribute('property'), 'og:') === 0) {
				$key = mb_strstr(mb_substr($tag->getAttribute('property'), 3), '-', '_');
